...
double electrostatics::operator()(size_t natom, const double* charge,
    ...
    const electrostatics::excluded_set_type& excluded, const smear& smr,
    double* grad)
{
    allocate(natom);
...
#   endif
    ...
    for (size_t i = 0; i < natom; ++i) {
        ...
        for (size_t j = i + 1; j < natom; ++j) {
            ...
            const bool ij_bonded =
              (excluded.find(std::make_pair(i, j)) != excluded.end());
            ...
            const double aDD = ij_bonded ? smr.aDD_intra() : smr.aDD_inter();
...
#   endif // DEBUG
    ...
    for (size_t i = 0; i < natom; ++i) {
        ...
        for (size_t j = 0; j < natom; ++j) {
            ...
            || (i < j ? excluded.find(std::make_pair(i, j)) != excluded.end()
                      : excluded.find(std::make_pair(j, i)) != excluded.end());
