...
contains
   ...
   subroutine force_mace(x, y, z, fx, fy, fz, eclas, walkmax)
      ...
      do iw = 1, walkmax
         ...
         success = receive_mace_data(fx, fy, fz, eclas, iw, walkmax, mace_comm)
#endif
         if (.not. success) then
   ...
   logical function receive_mace_data(fx, fy, fz, eclas, iw, walkmax, comm) result(success)
      ...
      if (.not. check_incoming_msg()) then
         success = .false.
         return
      end if
   ...
   end function receive_mace_data
