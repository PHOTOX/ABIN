#define REALLOCATE_LARGE_ARRAYS_EVERY_CALL
...
static PARMSTRUCT_T *prmqm;	//DHmod
...
static int *upairs_qm = NULL;      /* array of the number of pairs in upper triangle */
static int *lpairs_qm = NULL;      /* array of the number of pairs in lower triangle */
static int **pairlist = NULL;   /* array of pair list arrays */
static int **pairlist_qm = NULL;   /* array of pair list arrays */
static int nb_pairs = -1;       /* the total number of pairs */
static int nb_pairs_qm = -1;       /* DHmod the total number of pairs for QM part */
static int **N14pearlist = NULL;        /* array of N14 pair list arrays */
static int **N14pearlist_qm = NULL;        /* array of N14 pair list arrays */
...
//static int **IexclAt_qm = NULL;    /* array of excluded atom arrays for atom i */
...
static int npbverb = -9999, nsnbr = -9999, nsnba = -9999, npbgrid = -9999;
...
/* 3D RISM section  */
static RismData rismData={.rism=0,.closure=-9999,.closureOrder=-9999,.asympCorr=1,
                          .gauss_fluct=-9999,.solvcut=-9999,.buffer=-9999.,
			  .grdspc[0]=-9999.,.grdspc[1]=-9999.,.grdspc[2]=-9999.,
			  .ng3[0]=-9999,.ng3[1]=-9999,.ng3[2]=-9999,
			  .solvbox[0]=-9999.,.solvbox[1]=-9999.,.solvbox[2]=-9999.,
			  .tolerance=-9999.,.mdiis_del=-9999.,.mdiis_nvec=-9999,
			  .mdiis_method=-9999,.maxstep=-9999,.npropagate=-9999,.centering=-9999,
			  .zerofrc=-9999,.apply_rism_force=-9999,.polarDecomp=-9999,
                          .rismnrespa=-9999,.fcestride=-9999,
			  .fcecut=-9999.,.fcenbasis=-9999,.fcecrd=-9999,.saveprogress=-9999,
			  .ntwrism=-9999,.verbose=-9999,.progress=-9999};
...
#include "hcp.c"
#include "hcp_gb.c"
#include "eff_my_pme.c"
#if !defined(NOPERFLIB)
#  include "sff2_my_pme.c"
#endif
#include "mask.c"
...
{
   int i, j, at1, at2, iexcl, npairs,npairs_qm, *numb;
   ...
   //DH MOD: NASTY HACK FOR QMMM
   //we assume, if prm!=NULL,that whole system was allocated
   //and its time only for QM part

   if(prm != NULL) {
 	 prmqm = prm_in;


  	 upairs_qm = ivector(0, prmqm->Natom);
  	 lpairs_qm = ivector(0, prmqm->Natom);

         if ((pairlist = (int **) malloc(prmqm->Natom * sizeof(int *))) == NULL) {
        	 fprintf(nabout, "Error allocating pairlist_qm array in mme_init!\n");
     	     	 fflush(nabout);
      		 mpierror(-1);
  	 }

   	for (i = 0; i < prmqm->Natom; i++) {
        	pairlist_qm[i] = NULL;
      		lpairs_qm[i] = upairs_qm[i] = 0;
	 }

   	if ((N14pearlist_qm = (int **) malloc(prmqm->Natom * sizeof(int *))) == NULL) {
     	 fprintf(nabout, "Error allocating N14pearlist_qm array in mme_init!\n");
      	fflush(nabout);
      	mpierror(-1);
   	}
   	...
   	npairs_qm = 0;
   	for (i = 0; i < prmqm->Natom; i++) {
     	 N14pearlist_qm[i] = NULL;
     	 if (prmqm->N14pairs[i] > 0) {
        	 N14pearlist_qm[i] = ivector(0, prmqm->N14pairs[i]);
        	 for (j = 0; j < prmqm->N14pairs[i]; j++) {
           	 N14pearlist[i][j] = prmqm->N14pairlist[npairs + j];
         	}
         	npairs_qm += prmqm->N14pairs[i];
      }
   }

/*
   if ((IexclAt_qm = (int **) malloc(prmqm->Natom * sizeof(int *))) == NULL) {
      fprintf(nabout, "Error allocating IexclAt_qm array in mme_init!\n");
      fflush(nabout);
      mpierror(-1);
   }
   ...
   for (i = 0; i < prmqm->Natom; i++) {
      IexclAt_qm[i] = NULL;
      if (prmqm->Iblo[i] > 0) {
         IexclAt_qm[i] = ivector(0, prm->Iblo[i]);
         for (j = 0; j < prmqm->Iblo[i]; j++) {
            IexclAt[i][j] = prmqm->ExclAt[iexcl + j];
         }
      }
      iexcl_qm += prmqm->Iblo[i];
   }*/
        fprintf(nabout,"Halo from QM initialization\n");
   	return (0);
   }
  ...
  if(prm->IfBox ){
   fprintf(nabout, "Setting box sizes for PBC. \n");
   Lx_box=prm->Box[0];
   Ly_box=prm->Box[1];
   Lz_box=prm->Box[2];
   L2x=0.5*prm->Box[0];
...
#  endif
     ...
     rism_init_(&comm);
     if(ntpr_rism != 0)
       rism_thermo_(&t,&(rismData.polarDecomp),0);
   }
#else
   if ( rismData.rism ){
     fprintf(nabout,"Error: 3D-RISM not install.  Please recompile with -rismmpi.\n");
     fflush(nabout);
     mpierror(-1);
   }
#endif /*RISMSFF*/
  ...
  if (hcp == 0)    /* HCP uses it's own pairlist */
  {
   ...
   for (i = 0; i < prm->Natom; i++) {
      pairlist[i] = NULL;
      //DHmod: asi modifikovat tady,pro ipbc=1 dat upairs=1 vsem atomum?
      lpairs[i] = upairs[i] = 0;
   }
