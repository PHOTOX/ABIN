             ...
             int context_PxQ, int *descF_PxQ, int *descG_PxQ)
{
   ...
   REAL_T dx, dy, dz, gx, gy, gz, bi, bk, ct, boi2, boj2, z1, z2, bioj,
       bjoi;
   REAL_T ux, uy, uz, vx, vy, vz, dx1, dy1, dz1, delta;
   REAL_T phi, yy;
   REAL_T ct2, arg, z11, z12, z22, ct5, ct4, ct3;
...
/***********************************************************************
                            NBOND2()
DH: original function with no modifications
to be used if ipbc = 0 
************************************************************************/
           ...
           int context_PxQ, int *descF_PxQ, int *descG_PxQ)
{
   ...
   REAL_T dis, kij, d0, diff, rinv, rs, rssq, eps1, epsi, cgijr, pow, cgi;
 ...
 * initialized in nabinit_, defined in pme.h
...
static
int nbond2_pme(int *lpears, int *upears, int **pearlist, int N14,
           REAL_T * x, REAL_T * f, REAL_T * g, REAL_T * enb, REAL_T * eel,
           REAL_T enbfac, REAL_T eelfac,
           int context_PxQ, int *descF_PxQ, int *descG_PxQ)
{
   ...
   REAL_T xi, yi, zi, xij, yij, zij;
   REAL_T di[3], dj[3], d2ii[3][3], d2jj[3][3], d2ij[3][3], ijterm;
   REAL_T df2_pme,df_pme,rs_pme,er1,alfr2;
   //DHmod
   int myi;
...
#define C1 38.5
   ...
   *eel = 0.;
   ...
   for (i = 0; i < prm->Natom - 1; i++) {
      ...
      npr = upears[i];
      //DHmod: pro PBC s pouzitim nbond_box nemame upairs 
      //tj. budeme prohledavat vzdy cely pairlist
      //is this right?
      //we should rather fix nbond_pme.c
      
      if ( N14 == 0 ) {   // for nonbonded interactions
	      npr = lpears[i];
      }

      //fprintf(stdout,"upairs %i",npr);
      if (npr <= 0)
         continue;
      //fprintf(stdout,"NPR  %i \n",npr);
      ...
      //DHmod nemame upair, npr=lpears[i]=lpears+upairs
      lpair=0;
      ...
      for (jn = 0; jn < npr; jn++) {
         ...
         if (pearlist[i] == NULL) {
            printf("NULL pair list entry in nbond2, taskid = %d %i\n",
                   mytaskid,i);
            fflush(stdout);
         }
         j = pearlist[i][lpair + jn];
	  ...
	  //--this should be ok, if cutoff is less then L/2
	    if (xij < -L2x) xij+=prm->Box[0];
	    else if (xij > L2x) xij-=prm->Box[0];
	    if (yij < -L2y) yij+=prm->Box[1];
	    else if (yij > L2y) yij-=prm->Box[1];
	    if (zij < -L2z) zij+=prm->Box[2];
	    else if (zij > L2z) zij-=prm->Box[2];
   //      xij -= round( xij*boxxi )*prm->Box[0];
   //      yij -= round( yij*boxyi )*prm->Box[1];
   //      zij -= round( zij*boxzi )*prm->Box[2];
	  //---------------------------------------
	    }
            r2 = xij * xij + yij * yij + zij * zij;
            r2inv = 1.0 / r2;
		  ...
		  //alpha=0 gives clasical coulomb
		  //assumming that cut < L/2, otherwise we would have to sum over periodic images
		  //i didnt modify calculation of hessian, since it doesnt incoporates PME anyway
		  //adopted from  MACSIMUS routines for  real space part
#define ER 1.1283791670955126 /* 2/\pi * FROM ewaldf.c*/ 

                  rs = cgi * prm->Charges[j] * rinv;
                  df2 = -rs;		    // used for hessian
                  dg2 = 2.0 * rs * r2inv;   // only used for hessian, i.e. rs is used

		  if (ipbc == 1){
		     er1 = erfc(alpha_pme*r);
		     rs_pme = rs * er1;
		     //df2_pme is force term including PME,will be later multiplied by rinv*rinv
		     alfr2 = alpha_pme*alpha_pme*r*r;
	             df2_pme = -rs_pme - ER * alpha_pme * exp(-alfr2);
                     *eel += rs_pme;	    //DHMOD
		  } else {
		     df2_pme=df2;
                     *eel += rs;	    //DHMOD
		  ...
		  //THIS IS THE EQUATION FROM MACSIMUS
		  //r1=r,rr=r*r,Sqr -> ^2, 
	          //df2_pme=q[i]*q[j]/(rr*r1)*(ER*(el.alpha*r1)*exp(-Sqr(el.alpha*r1))+er1);
               ...
               if (ic > 0 || enbfac != 1.0) {
                  ...
                  *enb += (f1 - f2) * enbfaci;
                  df = (df2 + (6.0 * f2 - 12.0 * f1) * enbfaci) * rinv;
                  df_pme = (df2_pme + (6.0 * f2 - 12.0 * f1) * enbfaci) * rinv; // for updating gradient vector, includes erfc pme correction
                  dg = dg2 + (156.0 * f1 - 42.0 * f2) * enbfaci * r2inv;
#if 0
                  if (enbfac != 1.0)
                     nb14 += (f1 - f2) * enbfaci;
                  ...
                  *enb += (f1 - f2) * enbfaci;
                  df = (df2 + (10.0 * f2 - 12.0 * f1) * enbfaci) * rinv;
                  df_pme = (df2_pme + (10.0 * f2 - 12.0 * f1) * enbfaci) * rinv;
                  dg = dg2 + (156.0 * f1 - 110.0 * f2 * enbfaci) * r2inv;
                  nhbpair++;
#if 0
                  hbener += (f1 - f2) * enbfaci;
	    ...
	    //following is used for updating gradient on atom i t the and of the routine
	    //i.e. we use df_pme here
            dumx += df_pme * di[0];	
            dumy += df_pme * di[1];
            dumz += df_pme * di[2];
   ...
   /* Deallocate the iexw array. */
   /*DH, WARNING: THIS IS PROBABLY NOT THREAD SAFE!*/
...
static
REAL_T intersect2(INT_T * s, REAL_T * x, REAL_T * f, REAL_T * g,
                  REAL_T * rborn, REAL_T * area, INT_T n, INT_T gbsa,
                  int context_PxQ, int *descF_PxQ, int *descG_PxQ)
{
   int i, j, k, l, m, o, i3, j3, k3, a3;
   ...
   REAL_T ri, rj, rk, dij, dij2, r2inv, de, df, dg, cicjsci, kvgr3ci;
   REAL_T dedx, dedy, dedz, daix, daiy, daiz;
   REAL_T di[3], dj[3], d2ii[3][3], d2jj[3][3], d2ij[3][3], ijterm;
   ...
   if ((gbsa == 2) || (gbsa == 4)) {
      ...
      for (i = 0; i < n; i++) {
         ...
         for (j = i + 1; j < n; j++) {
            ...
            ptr = ptr1d(f, descF_PxQ, j3 + 0);
            if (ptr != NULL)
               *ptr -= dedx;
            ptr = ptr1d(f, descF_PxQ, j3 + 1);
            if (ptr != NULL)
               *ptr -= dedy;
            ptr = ptr1d(f, descF_PxQ, j3 + 2);
            if (ptr != NULL)
               *ptr -= dedz;
#else
            f[j3] -= dedx;
            ...
            /* Calculate df and dg for updating the Hessian matrix. */

            df = de;
            dg = 2.0 * cicjsci - r2inv;
            dg = 2.0 * cicjsci * dg * vg * surften;

            /* Calculate the second derivatives of Vg. */
            ...
            for (k = 0; k < 3; k++) {
               for (l = 0; l < 3; l++) {
                  ...
                  ptr = ptr2d(g, descG_PxQ, i3 + l, i3 + k);
                  if (ptr != NULL)
                     *ptr += df * d2ii[k][l] + dg * di[k] * di[l];
                  ptr = ptr2d(g, descG_PxQ, j3 + l, j3 + k);
                  if (ptr != NULL)
                     *ptr += df * d2jj[k][l] + dg * dj[k] * dj[l];
                  ptr = ptr2d(g, descG_PxQ, j3 + l, i3 + k);
                  if (ptr != NULL)
                     *ptr += ijterm;
                  ptr = ptr2d(g, descG_PxQ, i3 + k, j3 + l);
                  if (ptr != NULL)
                     *ptr += ijterm;
#else
                  g[i3 + k + a3 * (i3 + l)] +=
                  ...
                  g[j3 + k + a3 * (j3 + l)] +=
                      df * d2jj[k][l] + dg * dj[k] * dj[l];
                  g[i3 + k + a3 * (j3 + l)] += ijterm;
                  g[j3 + l + a3 * (i3 + k)] += ijterm;
#endif
               }
            }
         }
         ...
         ptr = ptr1d(f, descF_PxQ, i3 + 0);
         if (ptr != NULL)
            *ptr += daix;

         ptr = ptr1d(f, descF_PxQ, i3 + 1);
         if (ptr != NULL)
            *ptr += daiy;

         ptr = ptr1d(f, descF_PxQ, i3 + 2);
         if (ptr != NULL)
            *ptr += daiz;
   ...
   /* Is a surface area calculation requested? */
       ...
       * calculation that follows, m ranges over all atoms and results are summed.
       * Note: the index m is used instead of k because a k-loop follows.
       *
       * Each atom in the set takes its turn as the principal atom m.  This
      ...
      for (m = 0; m < o; m++) {
         ...
         /* Loop over all atoms except atom m to form sumcjdkj2. */
         ...
         for (i = 0; i < n; i++) {
            ...
            for (j = i + 1; j < n; j++) {
               ...
               dg -= cicjsci;
               dg *= (r2inv - 2.0 * cicjsci) * 2.0 * kvgr3ci * surften;
               dg -= 2.0 * cicjsci * (agk * surften * r2inv + de);

               /* Calculate the second derivatives of Ag. */
...
static
REAL_T atomset2(INT_T * setlist, INT_T start, INT_T finish, INT_T index,
                INT_T gbsa, INT_T * setarray, REAL_T * x, REAL_T * f,
                REAL_T * g, REAL_T * rborn, REAL_T * surfarea,
                INT_T * maxdepth, int context_PxQ, int *descF_PxQ,
                int *descG_PxQ)
{
   int j, i34, j34;
   REAL_T volume, totvolume, area, ri, rj;
   REAL_T xi, yi, zi, xj, yj, zj, xij, yij, zij, dij2;

   totvolume = 0.0;
   if (index >= max_set_size) {
      printf
          ("Overflow in atomset2: increase either max_set_size or min_volume!\n");
      return (totvolume);
   }
   if (*maxdepth < index) {
      *maxdepth = index;
   ...
   for (j = start; j < finish; j++) {
      ...
      volume = intersect2(setarray, x, f, g, rborn, &area, index + 1, gbsa,
      ...
      *surfarea += area;
      ...
      totvolume +=
          atomset2(setlist, j + 1, finish, index + 1, gbsa, setarray, x, f,
                   g, rborn, surfarea, maxdepth, context_PxQ, descF_PxQ,
                   descG_PxQ);
   }
   return (totvolume);
}
...
#define TLL 70.9090909090909090909
{
   ...
   /* AGBNP stuff follows */
   int maxdepth, maxmaxdepth, fpair;
   static int *setarray = NULL;
...
#define ROW_CYCLIC
   ...
   if ((psi == NULL) && (gb == 2 || gb == 5))
      psi = vector(0, n);
   if ((setarray == NULL) && (gbsa >= 2) && (gbsa <= 5))
...
#define GBGAMMA 2.909125
    ...
    *
    * Note: the first derivative code found in the gbsa.c file
    * computes the original LCPO approximation when gbsa==1.  This
    * difference between the first and second derivative calculation
    * should not be important because second derivatives are never
    * calculated for the LCPO approximation.
    */

   totalvolume = surfacearea = 0.0;
   if ((gbsa >= 2) && (gbsa <= 5)) {
       ...
       *
       * For the present, this computation is not parallelized.  A simple
       * approach to parallelization would be to provide a separate copy
       * of the gradient vector and Hessian matrix for each OpenMP thread
       * or MPI process, as is done for the gradient vector in the egb
       * function.  However, a separate copy of the Hessian matrix requires
       * too much memory for that approach to be viable.
       */

      maxmaxdepth = 0;
      for (i = 0; i < prm->Natom; i++) {

         /* Test whether the principal atom is frozen. */
         ...
         if (pearlistnp[i] == NULL) {
            fprintf(nabout,
                    "NULL non-polar pair list in egb2, taskid = %d\n",
                    mytaskid);
            fflush(nabout);
         }
         ...
         surfarea = 0.0;
         totvolume = atomset2(pearlistnp[i], fpair, npairs, 1, gbsa,
                              setarray, x, f, g, rborn, &surfarea,
                              &maxdepth, context_PxQ, desc_3Nx1,
                              desc_3Nx3N);
    ...
    * GB energy.  Note that this energy is not computed in parallel.
    */
...
//DH TODO: should deallocate arrays outside egb2 function
   ...
   if (nb_pairs < 0 || (*iter > 0 && *iter % nsnb == 0)) {
      ...
      *tmme2Other += t2 - t1;
      t1 = t2;
      //DHmod: we take nblist_box if we have box
      // although i modified nblist in nblist_pme.c for use with boxes, it doesn work correctly right now
      // perhaps i should fix it in the future for performance
      // but what about the upairs??
      if( prm->IfBox ){
	...
	*tmme2Other += t2 - t1;
	t1 = t2;
	np_pairs =
	  nblist(lpairsnp, upairsnp, pairlistnp, x, *context_PxQ, 0,
		 cutnp, prm->Natom, dim, frozen);
	t2 = seconds();
	*tmme2Pair += t2 - t1;
	t1 = t2;
      }
	...
	*tmme2Pair += t2 - t1;
	t1 = t2;
      ...
      if ((gbsa >= 2) && (gbsa <= 5)) {
         if (np_pairs2 < 0 || (*iter > 0 && *iter % nsnp == 0)) {
	   t2 = seconds();
	   *tmme2Other += t2 - t1;
	   t1 = t2;
	   np_pairs2 = nblist(lpairs2np, upairs2np, pairlist2np,
			      x, *context_PxQ, 0, cutnp, prm - Natom, dim,
			      frozen);
	   t2 = seconds();
	   *tmme2Pair += t2 - t1;
	   t1 = t2;
...
// DHmod 
   ...
   nbond2(lpairs, prm->N14pairs, N14pearlist, 1, x, f, g, &enb14, &eel14,
          scnb, scee, *context_PxQ, descF_PxQ, descG_PxQ );
   else
   nbond2_pme(lpairs, prm->N14pairs, N14pearlist, 1, x, f, g, &enb14, &eel14,
          scnb, scee, *context_PxQ, descF_PxQ, descG_PxQ );
...
#if defined(SCALAPACK) || defined(MPI)
   if ( ipbc == 0 )
      nbond2(lpairs2, upairs2, pairlist2, 0, x, f, g, &enb, &eel, 1.0, 1.0,
             *context_PxQ, descF_PxQ, descG_PxQ );
   else 
      nbond2_pme(lpairs2, upairs2, pairlist2, 0, x, f, g, &enb, &eel, 1.0, 1.0,
             *context_PxQ, descF_PxQ, descG_PxQ );
#else
   if ( ipbc == 0 )
     nbond2(lpairs, upairs, pairlist, 0, x, f, g, &enb, &eel, 1.0, 1.0,
             *context_PxQ, descF_PxQ, descG_PxQ);
   else
     nbond2_pme(lpairs, upairs, pairlist, 0, x, f, g, &enb, &eel, 1.0, 1.0,
             *context_PxQ, descF_PxQ, descG_PxQ);
#endif
