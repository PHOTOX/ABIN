 ...
 * DH note : when called from  function nblist, p=0 
 * DH note: I should test, how p changes during building of the kdtree
 *
 */
                  ...
                  REAL_T cut, REAL_T cut2, int dim, int *frozen)
{
    ...
    */
   // DH mod...added nearest neighbor
   //but not really, a dont know how..hopefully it should work if we wrap coordinates
   //xL=x[dim * q + p - 1]
   ...
   if ((q != that->n) && (!frozen[q] || !frozen[that->n])) {
	  ...
	  //--this should be ok, if cutoff is less then L/2+size of molecules
	  //this will be problematic for small boxes of larger molecules
...
#endif
   //DHMOD-uncomment for debugging
   //fprintf(stdout, "Number of pairs %i \n",totpair);
   return totpair;
}
