//DH:despite the name, no PME  buissness here, see eff_box
//for PBC simulations, we call nbond_box_pme
/* 
...
//DHMOD: TODO: zavolat tuhle rutinu s mensim nbond....mozna zjistit rucne?
//dalo by se zjisti nastenim input_qm.top,i kdyz bychom tu toppologii pak nevyuzili
...
//DHMOD: TODO: zavolat tuhle rutinu s mensim nangle....mozna zjistit rucne?
//dalo by se zjisti nastenim input_qm.top,i kdyz bychom tu toppologii pak nevyuzili
...
REAL_T mme34(REAL_T * x, REAL_T * f, int *iter)
{
   ...
   if (*iter == -3) {
     if (gb) {
       ...
       *tmmeOther += t2 - t1;
       t1 = t2;
       //DHmod: EPBSA currently not supported!
     ...
     t1 = t2;
      ...
      if( prm->IfBox ){
 	   ...
 	   nbond_box(lpairs, pairlist, x, grad, &enb, &eel );
           eexips( &enbips, &eelips, grad, x );
	} else {
 	  nbond_box_pme(lpairs, pairlist, x, grad, &enb, &eel );
	}
      ...
      //fprintf( nabout,"  enbips=%9.3f \n",enbips);
      //fprintf( nabout,"  eelips=%9.3f \n",eelips);
      ene[1] = enb + enbips;
      ene[2] = eel + eelips;
      ene[10] = 0.0;
      ene[11] = 0.0;
