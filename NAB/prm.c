...
/* hcp_preadln() - read to end-of-line; replace \n with \0 */

static void hcp_preadln(FILE * file, char *name, char *string, int type)
{
   int i, j;
   ...
   if (get_mytaskid() == 0) {
      for (i = 0; i < 81; i++) {
         if ((j = getc(file)) == EOF) {
            if (type != 0)    /* don't display error if looking for HCP pointer */
            {
               fprintf(nabout, "Error: unexpected EOF in %s\n", name);
            }
         } else {
...
/***************************************************************************
                             HCP_PFIND()
****************************************************************************/
int hcp_pfind(FILE *file, char *label, int type)
{
   char str[81];
   int flag=0;
   int i;
   ...
   /* initialize string */
   for (i = 0; i < 81; i++) { str[i] = ' '; }
   str[80] = '\0';

   /* skip end of line */
   while ((i = getc(file)) != 10) {
      if (i == EOF) {
         if (type != 0)   /* dont display error if looking for HCP pointer */
         {
            fprintf(nabout, "unexpected end in parm file\n");
         }
         break;
   ...
   hcp_preadln(file, label, str, type);
   if(!strncmp(str + 6, label, strlen(label)))
   {
      hcp_preadln(file, label, str, 1);   /* skip the format line */
      flag=1;
   }
...
/*
 * rdparm() - read a prmtop file and instantiate a PARMSTRUCT_T
 *    (Note: NAB's readparm() routine is just a wrapper calling this function)
 */
...
PARMSTRUCT_T* rdparm( STRING_T* name )
{
   ...
   if (get_mytaskid() == 0) {
      fprintf(nabout, "Reading parm file (%s)\n", name);
   }
   ier = 0;
   ...
   if (get_mytaskid() == 0) {
     if(!fscanf(file,
             ...
             &idum,        &idum,        &idum,       &prm->IfBox,
             &prm->Nmxrs,  &prm->IfCap)
       ) fprintf(nabout,"Warning:Cannot read input topology.");
   }
#if defined(MPI) || defined(SCALAPACK)
...
#endif
#if 0
         /* the NAB dihedral codes are not (yet) set up for arbitrary phases */
         assert( prm->Phase[i] == 0.0 || fabs(3.14159 - prm->Phase[i]) < 0.01 );
#endif
      }
   }
   ...
   if (!prm->IfBox) {
...
#if 0
      /* earlier versions of nab did not support periodic boundary cond.: */
      if (get_mytaskid() == 0) {
      ...
      if (get_mytaskid() == 0) {
         ...
         fscanf(file, " %lf %lf %lf %lf",
                &boxangle, &prm->Box[0], &prm->Box[1], &prm->Box[2]);
#else
         fscanf(file, " %f %f %f %f",
                &boxangle, &prm->Box[0], &prm->Box[1], &prm->Box[2]);
#endif
...
#define BOFFSET 0.09
   if (newparm) {
      ...
      if( !prm->IfBox ) {  /* this if's compound statement is not indented */
      if (get_mytaskid() == 0) {
         fprintf(nabout,
   ...
   if (get_mytaskid() == 0) {
      flag=hcp_pfind(file, "POINTERS FOR STRAND AND COMPLEX", 0);
      ...
      if(!flag) {
         ...
         fscanf(file,"%d%d",&prm->Ncomplex, &prm->Nstrand);
      }
   }

#if defined(MPI) || defined(SCALAPACK)
   MPI_Bcast(&prm->Ncomplex, 1, MPI_INT, 0, MPI_COMM_WORLD);
   MPI_Bcast(&prm->Nstrand, 1, MPI_INT, 0, MPI_COMM_WORLD);
#endif


   prm->Ipcomplex = (int *) get(sizeof(int) * (prm->Ncomplex + 1));
   prm->Ipstrand = (int *) get(sizeof(int) * (prm->Nstrand + 1));
   ...
   if (get_mytaskid() == 0) {
      if(!flag) {
         ...
         if (hcp_pfind(file, "STRAND_POINTER", 1)) {
            for (i = 0; i < prm->Nstrand; i++) {
               fscanf(file, " %d", &prm->Ipstrand[i]);
            }
         }
         else {
            fprintf(stderr, "HCP ERROR: STRAND_POINTER not found in prmtop file\n");
         }

         if (hcp_pfind(file, "COMPLEX_POINTER", 1)) {
            for (i = 0; i < prm->Ncomplex; i++) {
               fscanf(file, " %d", &prm->Ipcomplex[i]);
            }
         }
         else {
            fprintf(stderr, "HCP ERROR: COMPLEX_POINTER not found in prmtop file\n");
         }
      }
   }

#if defined(MPI) || defined(SCALAPACK)
   MPI_Bcast(prm->Ipcomplex, prm->Ncomplex, MPI_INT, 0, MPI_COMM_WORLD);
   MPI_Bcast(prm->Ipstrand, prm->Nstrand, MPI_INT, 0, MPI_COMM_WORLD);
#endif

   /********* End of changes for HCP implementation **********/
...
/***************************************************************************
			     WRPARM()
    this routine is not currently used directly in SFF,
    but it is used in NAB.
****************************************************************************/

/*
 * wrparm() - write a PARMSTRUCT_T (but only if task is master)
 */
...
/*
 * free_prm() - free space from an existing PARMSTRUCT_T
 */
